...
const CategoryForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchLanguages = async () => {
      try {
        const res = await getFromApi(`Category/get-categoryType-ddl`);
        setBuildings(res);
      } catch (error) {
        //console.log(error);
      }
  ...
  const onFinish = async (data) => {
    ...
    try {
      ...
      if (toEdit) {
        res = await postToApi(`Category/update-category-type`, payload);
      } else {
        res = await postToApi(`Category/add-category-type`, payload);
      }
      if (res) {
