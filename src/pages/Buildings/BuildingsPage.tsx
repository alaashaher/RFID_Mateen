...
import {
  ...
  Popconfirm,
  Modal,
  Tabs, Flex, Select, Input
} from "antd";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faCheck, faXmark } from "@fortawesome/free-solid-svg-icons";
import BuildingsContext from "../../contexts/pages-context/BuildingsProvider";
...
const BuildingsPage = () => {
    ...
    const getAllData = async () => {
      try {
        const resp = await getFromApi(
          `Building/get-all-Building-pager?isActive=${isActive}&pageSize=${pageSize}&currentPage=${pageNumber}&keyword=${keyword}`
        );
        setRowData(resp);
      } catch (error) {
        //console.log(error);
  ...
  const handleEditMod = async (TableId) => {
    try {
      const response = await getFromApi(
        `Building/get-building-by-id?buildingId=${TableId}`
      );
      if (response) {
        setToEdit(response);
  ...
  const handleDelete = async (TableId: number) => {
    try {
      setLoading(true);
      await deleteFromApi(
        `Building/delete-building?buildingId=${TableId}`
      );
      ...
      render: (_, record) => {
        return (
          <div className="act-btns">
            {user.user.Permissions.includes("EditBuildings") && (
              <Tooltip title="تعديل">
                <Button
                  onClick={() => {
                    handleEditMod(record.BuildingId);
                  }}
                  icon={<EditOutlined />}
                  shape="circle"
                />
            ...
            {user.user.Permissions.includes("DeleteBuildings") && (
              <Popconfirm
                title="هل أنت متأكد من الحذف؟"
                onConfirm={() => {
                  handleDelete(record?.BuildingId);
                }}
                okText="نعم"
                cancelText="لا"
              >
  ...
  const exportToCSV = () => {
    ...
    // const csvHeader = selectedColumns.map(col => `"${col.title.replace(/"/g, '""')}"`).join(',') + '\n';
    const selectedColumns = columns.slice(1, -1); // exclude first and last
    ...
    a.href = url;
    a.download = 'bulidings.csv';
    a.click();
    URL.revokeObjectURL(url);
  };
  return (
    <div className="custom-container">
      <h5 style={{ justifySelf: 'center', marginBottom: '20px' }}> المباني</h5>
      <div className="sec-dv sp-btwn">
        {user.user.Permissions.includes("AddBuildings") && (
          <Button
            onClick={() => {
              setOpenFormModel(true);
            }}
      ...
      <div className='sp-btwn'>
        ...
        <div className='sp-btwn'>
          <Flex
            gap="4px"
            align='center'>
            <Input type='text' style={{ width: 530 }} placeholder='ابحث باسم المنبي أو كود المبني' onChange={(e) => handleSearch(e)} />
            <span> </span>
          </Flex>
      ...
      {openFormModel && (
        <Modal
          width={"52%"}
          open={openFormModel}
          title={toEdit ? "تعديل مبني" : "اضافة مبني"}
          footer={false}
